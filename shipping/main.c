...
void init_assets(SDL_Renderer *renderer, Assets* A) {
    ...
    A->Back_1->dst = (SDL_Rect){.x=-WINDOW_WIDTH, .y=WINDOW_HEIGHT - SOIL_HEIGHT - SOIL_Y, .h=SOIL_HEIGHT, .w=WINDOW_WIDTH};
    ...
    A->Back_2->dst = (SDL_Rect){.x=0, .y=WINDOW_HEIGHT - SOIL_HEIGHT - SOIL_Y, .h=SOIL_HEIGHT, .w=WINDOW_WIDTH};
...
float get_angle2(int ax, int ay, int bx, int by) {
    int dist_mx = bx - ax;
    int dist_my = -(by - ay);
    float dist_x_norm = (float)dist_mx/(abs(dist_mx) + abs(dist_my));
    float angle;
    if (dist_my < 0) {
        angle = acosf(dist_x_norm)/PI*180;
    } else {
        angle = 360.0f - acosf(dist_x_norm)/PI*180;
    }
    return angle;
}
...
void display_dino_back_gun_cloud(State *state, SDL_Renderer *renderer, DArrayOfEntities *DAE, Assets *A) {
    ...
    Asset *arrayOfAssets[5] = {A->Back_1, A->Back_2, A->Back_3, A->Dino, A->Gun};
    for (int x=0; x<5; x++) {
        Asset *ptr = arrayOfAssets[x];
...
void manage_events(State* state, Assets* A, DA *DAE) {
    SDL_Event event;
            ...
            case SDL_KEYDOWN:
                switch (event.key.keysym.scancode) {
                    case SDL_SCANCODE_SPACE:
                        ...
                        if (state->AMMO > 0 && !state->GAMEOVER) {
                            state->AMMO--;
                            spawn_bullet(A, DAE);
                        }
                        break;
...
int main(void) {
    ...
    while (!GameState.CLOSE) {
        ...
        manage_events(&GameState, &GameAssets, &Bullets);
